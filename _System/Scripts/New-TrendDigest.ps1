...
param(
    # Defaults to the vault this script lives in (_System\Scripts\..\.. = vault root).
...
$TopN = 8
$DigestFolder = if (Test-Path (Join-Path $VaultPath "03-Resources\Research")) {
    Join-Path $VaultPath "03-Resources\Research\Trend-Digest"
} else {
    Join-Path $VaultPath "Research\Trend-Digest"
}
